...
async def enqueue_invite_email(email: str, full_name: str, tenant_id: str,
                               invite_token: str, expires: str) -> None:
    pool = await arq_pool()
    await pool.enqueue_job(
        "send_invite_email",
        email=email, full_name=full_name, tenant_id=tenant_id,
        invite_token=invite_token, expires=expires,
    )
...
async def enqueue_audit_export(job_id: str, tenant_id: str, from_ts: str | None,
                               to_ts: str | None, action: str | None) -> None:
    pool = await arq_pool()
    await pool.enqueue_job(
        "audit_export", job_id=job_id, tenant_id=tenant_id,
        from_ts=from_ts, to_ts=to_ts, action=action,
    )


# ---------- job implementations (worker side) ----------

async def _finish_job(db, job, status: str, result: dict | None = None,
                      error: str | None = None, row_count: int = 0) -> None:
    job.status = status
...
async def send_invite_email(ctx, email: str, full_name: str, tenant_id: str,
                            invite_token: str, expires: str) -> None:
...
async def audit_export(ctx, job_id: str, tenant_id: str, from_ts: str | None,
                       to_ts: str | None, action: str | None) -> None:
    import csv
    import io
    ...
    async with platform_session() as db:
        ...
        rows = (
            await db.execute(
                select(AuditLog).where(*cond).order_by(AuditLog.created_at).limit(200_000)
            )
        ).scalars().all()
        ...
        writer.writerow(
            ["id", "created_at", "actor_type", "actor_id", "actor_email",
             "action", "resource_type", "resource_id", "ip", "details"]
        )
        for r in rows:
            writer.writerow([
                r.id, r.created_at.isoformat(), r.actor_type, r.actor_id, r.actor_email,
                r.action, r.resource_type, r.resource_id, r.ip,
                __import__("json").dumps(r.details),
            ])
        key = f"{tid}/exports/audit-{job_id}.csv"
        s3_client().put_object(
            Bucket=settings.s3_bucket, Key=key, Body=buf.getvalue().encode(),
